...
export function formatProResult(result, options = {}) {
  const c = makeColors(options);
  const label = (s) => c.boldCyan(s);

  const lines = [];
  lines.push(`${label('Dialect: ')} ${c.bold(result.dialect)}`);
  lines.push(`${label('Pattern: ')} ${c.bold(result.expression)}`);
  lines.push(`${label('Meaning: ')} ${result.meaning}`);

  if (result.warnings?.length) {
    lines.push('');
    lines.push(label('Warnings'));
    for (const w of result.warnings) lines.push(`  ${c.yellow('⚠')}  ${w}`);
  }
  if (result.notes?.length) {
    lines.push('');
    lines.push(label('Notes'));
    for (const n of result.notes) lines.push(`  ${c.dim('•')}  ${n}`);
  }
  lines.push('');
  lines.push(`${label('Docs:    ')} ${c.dim(result.docs)}`);
  return lines.join('\n');
}

/** Pro result as plain JSON. */
export function formatProJson(result) {
